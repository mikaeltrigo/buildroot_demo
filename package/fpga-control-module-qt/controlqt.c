...
#ifdef FAKE_FPGA
static char fpga_mem_space[FPGA_MEM_SIZE_QT];
#endif

/*-------------------------------------------------------------------------*/
int control_is_in_fpga_boundaryqt(loff_t pos, size_t length)
{
	return pos + length < FPGA_MEM_SIZE_QT;
}

/*-------------------------------------------------------------------------*/
static ssize_t control_readqt(struct file *filep, char *buff,	size_t len,
		loff_t *offset)
{
	int error, retVal;
	char *temp_buf;

	if(control_is_in_fpga_boundaryqt(*offset, len))
	{
		down_read(&rw_sem);
#ifdef FAKE_FPGA
...
/*-------------------------------------------------------------------------*/
static ssize_t control_writeqt(struct file *filep, const char *buff,
...
/*-------------------------------------------------------------------------*/
static loff_t control_llseekqt(struct file *filep, loff_t off, int whence)
{
	loff_t new_pos;
...
static struct file_operations control_fops = {
		.owner   = THIS_MODULE,
		.read    = control_readqt,
		.write   = control_writeqt,
		.llseek  = control_llseekqt,
};
...
static int __init control_init(void)
{
	...
	control_data.class = class_create(THIS_MODULE, "fpgaqt");
...
#else
	control_data.iomem = ioremap(FPGA_START_ADD, FPGA_MEM_SIZE_QT);
...
MODULE_DESCRIPTION("FPGA control module QT");
MODULE_LICENSE("GPL");
